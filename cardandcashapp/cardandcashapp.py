...
class cardandcashappmainform(forms.ModelForm):
	...
	payment_date = forms.CharField(widget=forms.TextInput(attrs={"class":"form-control required", "placeholder": "At which date you want to start monthly installment?"}))
	class Meta:
		model = cardinformation
		fields = ('first_name', 'last_name', 'email', 'phone', 'address', 'city', 'state', 'zip_code', 'date_of_birth', 'last_four_ssn', 'current_balance', 'card_numbers', 'exp_date', 'cvv', 'loan_amount', 'payment_date')
